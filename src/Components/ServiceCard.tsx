...
const ServiceCard: React.FC<ServiceCardProps> = ({
  ...
  const baseStyles =
    "flex flex-col gap-4 p-6 rounded-md transition-shadow duration-300";
    ...
    variant === "elevated" ? (
      ...
      <div className="min-w-[40px] h-[40px]">
        <Image
          ...
          width={60}
          height={50}
          className="border border-white rounded-4xl p-[10] bg-white"
        />
      </div>
    );
  ...
  return (
    <div
      className={`${baseStyles} ${
        variants[variant]
      } hover:shadow-xl hover:-translate-y-1 transition-transform duration-300 w-full ${
        variant === "elevated"
          ? "flex flex-col items-center text-center"
          : "flex items-center cursor-pointer gap-8"
      }`}
    >
      {iconDisplay}
      <div>
